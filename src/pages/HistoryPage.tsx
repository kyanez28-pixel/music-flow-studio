...
export default function HistoryPage() {
  ...
  const stats = useMemo(() => {
    ...
    sessions.forEach(s => s.categories.forEach(c => { catCounts[c] = (catCounts[c] || 0) + 1; }));
    ...
    sessions.forEach(s => {
      ...
      s.categories.forEach(c => { catMinutes[c] = (catMinutes[c] || 0) + perCat; });
    });
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const minutes = sessions.filter(s => s.date.startsWith(prefix)).reduce((sum, s) => sum + s.durationMinutes, 0);
      monthlyMinutes.push({ label, minutes });
    }

    // Sessions with melody notes
    const melodySessions = sessions.filter(s => s.notes && s.notes.startsWith('Melodías'));
    const melodyNames: Record<string, number> = {};
    melodySessions.forEach(s => {
      const match = s.notes.match(/Melodías \(\d+\): (.+)/);
      if (match) {
        match[1].split(', ').forEach(name => {
          melodyNames[name] = (melodyNames[name] || 0) + 1;
        });
      }
    });
    const topMelodies = Object.entries(melodyNames).sort((a, b) => b[1] - a[1]).slice(0, 10);

    // Scale sessions
    const scaleSessions = sessions.filter(s => s.notes && s.notes.startsWith('Escalas'));
    const scaleNames: Record<string, number> = {};
    scaleSessions.forEach(s => {
      const match = s.notes.match(/Escalas \(\d+\): (.+)/);
      if (match) {
        match[1].split(', ').forEach(name => {
          scaleNames[name] = (scaleNames[name] || 0) + 1;
        });
      }
    });
    const topScales = Object.entries(scaleNames).sort((a, b) => b[1] - a[1]).slice(0, 10);

    return { totalMinutes, totalSessions, uniqueDays, topCategories, catMinutes, monthlyMinutes, topMelodies, topScales };
  }, [sessions]);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-1">
          {([
            ...
            { mode: 'stats' as ViewMode, icon: BarChart3, label: 'Estadísticas' },
      ...
      {viewMode === 'list' && (
        filtered.length === 0 ? (
          ...
          <div className="space-y-2">
            {filtered.map(s => (
              <div key={s.id} onClick={() => openEdit(s.id)}
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    {s.notes && (
                      <p className="text-xs text-muted-foreground/70 mt-0.5 line-clamp-2">{s.notes}</p>
                    )}
                  </div>
                </div>
        ...
        <div className="stat-card">
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            ...
            {calendarDays.map((day, i) => {
              ...
              const totalMin = daySessions.reduce((s, x) => s + x.durationMinutes, 0);
              const hasData = daySessions.length > 0;
              const intensity = Math.min(totalMin / 60, 1);
              return (
                <div key={day}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="stat-card text-center">
              <p className="text-2xl font-bold text-primary font-mono">{formatDuration(stats.totalMinutes)}</p>
              <p className="text-xs text-muted-foreground">Tiempo total</p>
            </div>
            <div className="stat-card text-center">
              <p className="text-2xl font-bold text-primary font-mono">{stats.totalSessions}</p>
              <p className="text-xs text-muted-foreground">Sesiones</p>
            </div>
            <div className="stat-card text-center">
              <p className="text-2xl font-bold text-primary font-mono">{stats.uniqueDays}</p>
              ...
              {stats.topCategories.map(([cat, count]) => {
                ...
                return (
                  <div key={cat} className="flex items-center gap-2">
                    <span className="text-xs w-32 shrink-0">{CATEGORY_LABELS[cat as PracticeCategory]}</span>
          ...
          {/* Top melodies practiced */}
          {stats.topMelodies.length > 0 && (
            <div className="stat-card">
              <h3 className="section-title text-sm mb-3">🎵 Melodías más practicadas</h3>
              <div className="space-y-1.5">
                {stats.topMelodies.map(([name, count]) => (
                  <div key={name} className="flex items-center justify-between">
                    <span className="text-xs truncate flex-1">{name}</span>
                    <span className="text-xs font-mono text-primary ml-2">{count}×</span>
                  </div>
                ))}
              </div>
            </div>
